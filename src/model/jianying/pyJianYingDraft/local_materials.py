...
class VideoMaterial:
    ...
    def __init__(self, path: str, material_name: Optional[str] = None, crop_settings: CropSettings = CropSettings(),
        ...
        Args:
            ...
            duration (`int`, optional): 素材时长(微秒), 如果指定则跳过文件解析, 用于性能优化.
            width (`int`, optional): 素材宽度, 如果指定则跳过文件解析.
            height (`int`, optional): 素材高度, 如果指定则跳过文件解析.
        ...
        # 如果手动提供了所有参数，跳过文件检查和解析（性能优化）
        if duration is not None and width is not None and height is not None:
            self.duration = duration
            self.width = width
            self.height = height
            # 根据扩展名判断类型
            image_formats = ['.jpg', '.jpeg', '.png', '.bmp', '.webp', '.gif']
            self.material_type = "photo" if postfix in image_formats else "video"
        else:
            # 需要解析文件时，转换为绝对路径并检查文件是否存在
